    ...
    <div>
        <ul class="flex space-x-2 rtl:space-x-reverse">
            ...
            <li class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1">
                <a href="{{ route('room.index') }}" class="text-primary hover:underline">Kamar</a>
            </li>
            <li class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1">
                    ...
                    <div>
                        ...
                        <div>
                            <form class="space-y-5" method="POST"
                                action="{{ isset($room) ? route('room.update', $room) : route('room.store') }}"
                                    ...
                                    <select class="homestaySelect selectize form-select form-select-lg text-white-dark"
                                        ...
                                        @if (isset($room))
                                            @foreach ($homestays as $homestay)
                                                @if ($room->homestay == $homestay->code)
                                ...
                                <div class="@error('roomtype')  has-error @enderror">
                                    <label for="roomtype">Pilih Tipe Kamar</label>
                                    <select class="selectize form-select form-select-lg text-white-dark" id="roomtype"
                                        name="roomtype">
                                        <option value="">Pilih Tipe Kamar</option>
                                        @if (isset($type))
                                            @foreach ($roomTypes as $code => $name)
                                                <option value="{{ $code }}"
                                                    {{ $room->room_type == $code ? 'selected' : '' }}>
                                                    {{ $name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                ...
                                <div class=" @error('room_number')  has-error @enderror">
                                    <label for="room_number">Nomor Kamar</label>
                                    <input id="room_number" name="room_number" type="text" required
                                        placeholder="Masukkan Nomor Kamar" class="form-input"
                                        value="{{ isset($room) ? $room->room_number : old('room_number') }}" />
                                    @error('room_number')
                                        <div class="text-danger mt-1">{{ $message }}</div>
                                    @enderror
                                </div>
        ...
        @php
            $validFrom = isset($ticket) ? $ticket->valid_from : old('valid_from');
            $validTo = isset($ticket) ? $ticket->valid_to : old('valid_to');
        @endphp
        <!-- start hightlight js -->
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                $('.homestaySelect').change(function() {
                    var homestayId = $(this).val();
                    if (homestayId) {
                        $.ajax({
                            url: '{{ route('homestay.roomtypes') }}',
                            type: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}',
                                code: homestayId
                            ...
                            success: function(data) {
                                $('#roomtype').empty();
                                $('#roomtype').append('<option value="">Select Room Type</option>');
                                $.each(data, function(code, name) {
                                    $('#roomtype').append('<option value="' + code + '">' +
                                        name + '</option>');
                                });
                            }
                        });
                ...
                Alpine.data('form', () => ({
                    ...
                    init() {
                        ...
                        flatpickr(document.getElementById('range-calendar'), {
                            dateFormat: 'Y-m-d',
                            mode: 'range',
                            defaultDate: [vm.validFrom, vm.validTo],
                            onChange: function(selectedDates, dateStr, instance) {
                                if (selectedDates.length === 2) {
                                    vm.validFrom = selectedDates[0].toISOString().split('T')[0];
                                    vm.validTo = selectedDates[1].toISOString().split('T')[0];
                                } else {
                                    vm.validFrom = null;
                                    vm.validTo = null;
                                }
