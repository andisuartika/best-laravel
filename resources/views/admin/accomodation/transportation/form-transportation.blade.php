    ...
    <div>
        <ul class="flex space-x-2 rtl:space-x-reverse">
            ...
            <li class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1">
                <a href="{{ route('transportations.index') }}" class="text-primary hover:underline">Transportasi</a>
            </li>
            <li class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1">
                <span>
                    ...
                    <div>
                        ...
                        <div>
                            <form class="space-y-5" method="POST"
                                action="{{ isset($transportation) ? route('transportations.update', $transportation) : route('transportations.store') }}"
                                ...
                                <div class=" @error('name')  has-error @enderror">
                                    ...
                                    <input id="name" name="name" type="text" required
                                        placeholder="Masukkan Nama Transportasi" class="form-input"
                                        value="{{ isset($transportation) ? $transportation->name : old('name') }}" />
                                    @error('name')
                                        <div class="text-danger mt-1">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class=" @error('price')  has-error @enderror">
                                    <label for="price">Harga Transportasi (/10Jam)</label>
                                    <div class="flex">
                                        <div
                                        ...
                                        <input type="text" placeholder="Masukkan Harga Transportasi"
                                            ...
                                            value="{{ isset($transportation) ? rtrim(rtrim($transportation->price, '0'), '.') : old('price') }}"
                                            required />
                                    </div>
                                    @error('price')
                                        <div class="text-danger mt-1">{{ $message }}</div>
                                ...
                                <div class=" @error('extra_price')  has-error @enderror">
                                    <label for="extra_price">Harga Tambahan (/Jam)</label>
                                    <div class="flex">
                                        <div
                                            class="bg-[#eee] flex justify-center items-center ltr:rounded-l-md rtl:rounded-r-md px-3 font-semibold border ltr:border-r-0 rtl:border-l-0 border-[#e0e6ed] dark:border-[#17263c] dark:bg-[#1b2e4b]">
                                            Rp</div>
                                        <input type="text" placeholder="Masukkan Harga Tambahan"
                                            class="form-input rounded-none" name="extra_price" id="extra_price"
                                            value="{{ isset($transportation) ? rtrim(rtrim($transportation->extra_price, '0'), '.') : old('extra_price') }}"
                                            required />
                                    </div>
                                    @error('extra_price')
                                        <div class="text-danger mt-1">{{ $message }}</div>
                                ...
                                <div class="@error('image')  has-error @enderror">
                                    ...
                                    <input id="ctnFile" type="file"
                                        ...
                                        {{ !isset($transportation) ? 'required' : '' }} />
                                    <div class="py-5">
                                        <img id="preview"
                                            src="{{ isset($transportation) ? asset($transportation->image) : asset('assets/images/file-preview.svg') }}"
    ...
    @endif
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                var extra_price = document.getElementById('extra_price');
                // Format value saat halaman pertama kali dimuat
                price.value = formatNumber(price.value);
                price.value = formatNumber(extra_price.value);
                price.addEventListener('keyup', function(e) {
                    price.value = formatNumber(price.value);
                });
                extra_price.addEventListener('keyup', function(e) {
                    extra_price.value = formatNumber(extra_price.value);
                });
            });
        </script>
</x-layout.default>
