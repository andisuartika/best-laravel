    ...
    <div>
                ...
                <div class="panel p-5">
                    <div>
                        <h4 class="mb-4 text-2xl font-semibold">
                            @if (isset($tour))
                                Ubah
                            @endif Paket Tour Wisata
                        </h4>
                        <p class="mb-4">
                            Silahkan lengkapi form berikut untuk
                            @if (isset($tour))
                                Ubah
                            @else
                                menambah
                        ...
                        <div>
                            <form class="space-y-5" method="POST"
                                action="{{ isset($tour) ? route('tours.update', $tour) : route('tours.store') }}"
                                ...
                                @else
                                    ...
                                    <div class="@error('manager')  has-error @enderror">
                                        <label for="manager">Pengelola Tour</label>
                                ...
                                <div class=" @error('name')  has-error @enderror">
                                    ...
                                    <input id="name" name="name" type="text" required
                                        placeholder="Masukkan Nama Paket Tour" class="form-input"
                                        value="{{ isset($tour) ? $tour->name : old('name') }}" />
                                    @error('name')
                                        <div class="text-danger mt-1">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="@error('destination') has-error @enderror">
                                    <label for="destination">Fasilitas Destinasi</label>
                                    <select class="destinationSelect form-select form-select-lg text-white-dark"
                                        name="destination[]" multiple="multiple">
                                        @if (isset($tour))
                                            @php
                                                // Konversi string JSON ke dalam array PHP
                                                $selectedDestination = json_decode($tour->destination);
                                            @endphp
                                            @foreach ($destinations as $destination)
                                                <option value="{{ $destination->code }}"
                                                    {{ in_array($destination->code, $selectedDestination) ? 'selected' : '' }}>
                                                    {{ $destination->name }}
                                                </option>
                                            @endforeach
                                        @endif
                                        @foreach ($destinations as $destination)
                                            <option value="{{ $destination->code }}"
                                                {{ collect(old('destination'))->contains($destination->code) ? 'selected' : '' }}>
                                                {{ $destination->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class=" @error('description')  has-error @enderror">
                                    <label for="description">Deskripsi Paket Tour</label>
                                    <textarea id="editor" name="description">{{ isset($tour) ? $tour->description : old('description') }}</textarea>
                                ...
                                <div class=" @error('included')  has-error @enderror">
                                    <label for="included">Paket Tour Include</label>
                                    @php
                                        $includedItems = [
                                            'Entrance fees',
                                        ...
                                        $includedValues = isset($tour) ? json_decode($tour->included, true) : [];
                                    @endphp

                                    @foreach ($includedItems as $item)
                                        <label class="flex items-center cursor-pointer">
                                            <input type="checkbox" name="included[]" value="{{ $item }}"
                                                class="form-checkbox"
                                                @if (in_array($item, $includedValues)) checked @endif />
                                            <span class="text-white-dark">{{ $item }}</span>
                                        </label>
                                    @endforeach
                                ...
                                <div class="@error('thumbnail')  has-error @enderror">
                                    <label for="ctnFile">Gambar Transportasi</label>
                                    ...
                                    <div class="py-5">
                                        <img id="preview"
                                            src="{{ isset($tour) ? asset($tour->thumbnail) : asset('assets/images/file-preview.svg') }}"
                                ...
                                <div class="space-y-4">
                                    <label class="block font-semibold">Harga Tour</label>

                                    <div id="tour-rates-section" class="space-y-4">
                                        @if (isset($tour) && $tour->rates)
                                            @foreach ($tour->rates as $i => $rate)
                                        ...
                                        @else
                                            ...
                                            <div class="tour-rate-group border p-4 rounded-md bg-white dark:bg-slate-800">
                                                <div class="flex flex-wrap gap-4 items-end">
                                                    <div class="flex-1 min-w-[180px]">
                                                        <label class="block mb-1 text-sm font-medium">Nama Harga</label>
        ...
        {{-- Tour Rate --}}
            <script>
                ...
                function addRate() {
                    const section = document.getElementById('tour-rates-section');
                    const newRate = document.createElement('div');
                    newRate.className = 'tour-rate-group border p-4 rounded-md bg-white dark:bg-slate-800 mb-2';
                    ...
                    newRate.innerHTML = `
                        <div class="flex flex-wrap gap-4 items-end">
                            ...
                            <div>
                                <button type="button" class="btn btn-danger" onclick="this.closest('.tour-rate-group').remove()">Hapus</button>
                            </div>
                        </div>
                    `;
